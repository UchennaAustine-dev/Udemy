...
export default function TrendingTopics() {
  const categories: TrendingCategory[] = [
    ...
    },
    {
      name: "Business",
      topics: [
        { name: "PMI Project Management Professional (PMP)", learners: "2,554,974 learners" },
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
      {categories.map((category, index) => (
        <div key={index}>
          <h3 className={`${index === 0 ? "text-2xl" : "text-xl"} font-bold mb-4 text-gray-800`}>{category.name}</h3>
          <div className="space-y-6">
            {category.topics.map((topic, topicIndex) => (
              <div key={topicIndex}>
                <Link href="#" className="flex items-center text-purple-700 font-medium mb-1 hover:underline">
